...
def embed_integrity(data: bytes, key: bytes) -> bytes:
    ...
    auth = pqs_hash(data + key, key)[:AUTHCODE_LENGTH]
    return check + auth + data

def verify_integrity(data: bytes, key: bytes) -> bytes:
    if len(data) < INTEGRITY_LENGTH + AUTHCODE_LENGTH:
        raise ValueError("Corrupted integrity")
    check = data[:INTEGRITY_LENGTH]
    auth = data[INTEGRITY_LENGTH:INTEGRITY_LENGTH + AUTHCODE_LENGTH]
    core = data[INTEGRITY_LENGTH + AUTHCODE_LENGTH:]
    expected_check = pqs_hash(core, key)[:INTEGRITY_LENGTH]
    expected_auth = pqs_hash(core + key, key)[:AUTHCODE_LENGTH]
    if check != expected_check or auth != expected_auth:
        raise ValueError("Integrity/auth check failed")
    return core

def int_to_bytes(i: int) -> bytes:
    return i.to_bytes(ORIGSIZE_LENGTH, 'big')
...
def pqs_encrypt(data: bytes, password: str) -> bytes:
    ...
    sized = int_to_bytes(len(data)) + masked
    secured = embed_integrity(sized, key)
    padded = apply_fakepad(secured, salt)
    return VERSION + salt + padded

def pqs_decrypt(encrypted: bytes, password: str) -> bytes:
    min_len = 4 + SALT_LENGTH + FAKEPAD_LENGTH * 2 + INTEGRITY_LENGTH + AUTHCODE_LENGTH + ORIGSIZE_LENGTH
    ...
    orig_size = bytes_to_int(core[:ORIGSIZE_LENGTH])
    masked = core[ORIGSIZE_LENGTH:]
    if len(masked) < orig_size:
        raise ValueError("Truncated payload")
    data = bytes([b ^ key[i % len(key)] for i, b in enumerate(masked)])
